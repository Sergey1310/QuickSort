...
std::vector<int> midll; // Для опорного элемента и его копий
...
std::vector<int> right; // И для чисел больше него
    ...
    for (int i : vec) { // Пробегаемся по вектору
        if (i == pivot) // Пивот и элементы равные ему отправляем в его собственный масив
            midll.push_back(i);
    ...
    left.insert(left.end(), midll.begin(),midll.end()); // Соединяем меньший вектор с массивом содержащий пивот и его копии
...
int main()
{
    std::vector<int> first{9,19,8,31,13,11,12,15,14,31,45,677,0,0,11,11,22,33,33,43};
    for(int i : first)
        std::cout << i << std::endl;
    Qsort(first);
